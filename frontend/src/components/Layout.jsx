...
export default function MiniDrawer() {
  ...
  const [selectedPage, setSelectedPage] = React.useState('Files')
  ...
  return (
    <Box sx={{ display: 'flex' }}>
      ...
      <Drawer variant="permanent" open={open}>
        ...
        <Divider />
        <List>
          {['Files', 'Passwords', 'Settings'].map((text, index) => (
        ...
        >
          {text === 'Files' && <InsertDriveFileIcon />}
          {text === 'Passwords' && <KeyIcon />}
        ...
        <Box component="main" sx={{ flexGrow: 1, p: 3 }}>
            <DrawerHeader />
            {selectedPage === 'Files' && <FilesPage />}
            {selectedPage === 'Passwords' && <div>Passwords coming soon</div>}
            {selectedPage === 'Settings' && <div>Settings coming soon</div>}
        </Box>
    </Box>
  );
}
